...
def choose(paragraphs, select, k):
    ...
    ''
    """
    qualified_paragraphs = [e for e in paragraphs if select(e) == True]
...
def about(topic):
    ...
    def helper(paragraph):
        # use functions provided by utils :)
...
def accuracy(typed, reference):
    ...
    # now, both of them are not empty
    correct_cnt = 0.
    # avoid illegal indices access
...
def wpm(typed, elapsed):
    ...
    # WARNING: the `word` here is not a real word but 5-character
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    diff_results = [diff_function(typed_word, word, limit) for word in valid_words]

    lowest_diff = min(diff_results)
    if lowest_diff > limit:
        return typed_word
    else:
        first_occur = diff_results.index(lowest_diff)
        return valid_words[first_occur]
...
def feline_flips(start, goal, limit):
    ...
    5
    """
    # The description says we should solve it recursively :)

    cnt = 0      # the diff cnt
    def helper(current, goal):
        nonlocal cnt
        # no need to go deeper
        if cnt > limit:
            return
        # base case 1. need len(goal) additiona
        if current == '':
            cnt += len(goal)
            return 
        # base case 2. need len(current) deletion
        if goal == '':
            cnt += len(current)
            return
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    # base case 1. they are same
    if start == goal: 
        return 0

    # base case 2. addition or deletion
    elif start == "" or goal == "":   
        return max(len(start), len(goal))
    # base case 3. no operations available
    elif limit == 0:
        return float('inf')
    else:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    ratio = correct_cnt / len(prompt)
    
    p = {'id': user_id, 'progress': ratio}
    upload(p)
    return ratio
...
def time_per_word(words, times_per_player):
    ...
    [[6, 3, 6, 2], [10, 6, 1, 2]]
    """
    # times[i][j] indicates how long it took player i to type words[j].
...
def fastest_words(match):
    ...
    for word_idx in word_indices:
        current_word = word_at(match, word_idx)

        # find the min value along with the current column
        min_plalyer, min_value = 0, float('inf')
        for player in player_indices:
            if times[player][word_idx] < min_value:
                min_value = times[player][word_idx]
                min_plalyer = player
        results[min_plalyer].append(current_word)
    return results
