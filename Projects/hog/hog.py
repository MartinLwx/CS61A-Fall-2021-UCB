...
def picky_piggy(score):
    ...
    if score == 0:
        return 7
    digits = int(1 / 7 * 10 ** 6)       # get 142857
    mod_result = score % 6

    # he decimal expansion of 1/7 is a 6-digit repeating decimal with 
    # the digits 142857
    # It is sillyl but it works though :)
    if mod_result == 0:
        return 7
    elif mod_result == 1:
        return 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal or score1 < goal:
        if who == 0:
            ...
            score1 += take_turn(num_rolls1, score0, dice, goal)
    ...
    def helper(score0, score1):
        if who == 0:
            if score0 - last_score > running_high:
                print(f"{score0 - last_score} point(s)! That's a record gain for Player {who}!")
            return announce_highest(who, score0, max((score0 - last_score), running_high))
        else:
            if score1 - last_score > running_high:
                print(f"{score1 - last_score} point(s)! That's a record gain for Player {who}!")
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    if hog_pile((picky_piggy(opponent_score) + score), opponent_score) != 0:
        return 0
